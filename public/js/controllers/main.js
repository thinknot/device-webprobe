angular.module('testrunController', [])
    ...
    .controller('mainController', ['$scope','$http','Testruns','TestExecutor', 
				   function($scope, $http, Testruns, TestExecutor) {
		...
		$scope.createTestrun = function() {
			...
			// call the test API to kick off the test
			TestExecutor.create($scope.formData)
			.success(function(data) {

		    	    // now create the database record service (returns a promise object)
